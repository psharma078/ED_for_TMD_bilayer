...
def main(theta, Nup, Vd):
    ...
    results = Parallel(n_jobs=48)(delayed(solver)(dk) for dk in delta_klist)
...
def manybody_Hamiltonian(Len, configs_indx, up_configs, singleE, k1234, V1234):
    ...
    if Len==1:
        ...
        idx = Eigs.argsort()
        Eigs = Eigs[idx]
        Evecs = [vecs[:,ID] for ID in idx]
    elif Len>1:
        Eigs, Evecs = eigsh(matrix,k=1,which='SA',sigma=None,return_eigenvectors=True)
        Evecs = Evecs.flatten()
    return Evecs
...
Nhole = 9
Vd = 0.0 #Displacement field

F12, dtheta, ktot =  main(theta, Nhole, Vd)
...
fig.savefig('manybody_BerryCur_27sites_nh='+str(Nhole)+'_ep='+str(epsilon_r)+'_theta='+str(round(theta,2))+'P=0.png',bbox_inches='tight',dpi=300)
fig.savefig('manybody_BerryCur_27sites_nh='+str(Nhole)+'_ep='+str(epsilon_r)+'_theta='+str(round(theta,2))+'P=0.pdf',bbox_inches='tight',dpi=300)
