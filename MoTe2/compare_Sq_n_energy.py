...
def main(theta,N1,N2,Nup,Vd):
    ...
    Evals = []
    singleE=singleE*0 
    Gs_Ktot_ind = [0] ##insert the k index where gs degeneracy happens
    ...
    def CSF(qval, evec, config, klist, Len, Fmat):#same order as Hamiltonian
        ...
        for i, k2 in enumerate(k2_ind):
            Lambda_k2 = Fmat[gk2_m_q_id[i]*n_k34**2 + k2*n_k34 + k2_minus_q_id[i]]
            valid_config_k2ID = (getspin(config, k2)==1).nonzero()[0]
            ...
            for j, k1 in enumerate(k1_ind):
                ...
                orginal_idlist = valid_config_k2ID[valid_config_k2mqID[valid_config_k1ID[valid_config_k1pqID]]]
                newconfig_id = np.array([config_lookup[nc] for nc in newconfig]).astype(int)

                Lambda_k1 = Fmat[gk1_p_q_id[j]*n_k34**2 + k1*n_k34 + k1_plus_q_id[j]]
                if Len == 1:
                    S_q += Lambda_k1*Lambda_k2 * sum(evec[newconfig_id].flatten().conjugate() * evec[orginal_idlist].flatten() * fsign)
                else:
                    S_q += Lambda_k1*Lambda_k2 * sum(evec[newconfig_id].flatten().conjugate() * evec[orginal_idlist].flatten() * fsign)
    ...
    def CSF_for_degenrateGS(qval, evecs, configs, klist, Fmat,vc):
        Len = len(configs)
        Sq = 0
        if Len==1:
            Sq = 0.5*vc*CSF(qval, evecs, configs[0], klist, Len, Fmat).real
            #Sq = CSF(qval, evecs, configs[0], klist, Len, Fmat).real
        else:
            for p in range(1):
        ...
        return Sq
    ##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    solver = lambda q: CSF_for_degenrateGS(qlist[q], Evecs, configurs, klist, Fmat_up, Vlist[q])
    Sq =  np.array(Parallel(n_jobs=num_cores)(delayed(solver)(q) for q in range(len(qlist))))
    print("sum of sq=", sum(Sq))
    Sq[0] = Sq[0]-Nup*Nup
    Sq = Sq/(N1*N2)
...
def plot_nk_in_hBz(N1, N2, ktot, nk, Label):
...
ktot, Sq =  main(theta, N1, N2, Nup, Vd)

#figo, axo = plot_charge_density(nk, ktot, Nup)
#fig_nk, ax_nk = plot_nk_in_hBz(N1, N2, ktot, nk, r'$n_k$')
fig, ax = plot_nk_in_hBz(N1, N2, ktot, Sq, r'$S(q)/S_o$')

#fig_yang,ax_yang = plot_ED_energies(Ktot_yang, Evals_yang)
