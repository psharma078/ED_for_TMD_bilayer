...
def main(theta,N1,N2,Nup,Vd):
    ...
    Gs_Ktot_ind = [0,0,0] ##insert the k index where gs degeneracy happens
    ...
    ## Calculating charge occupation <nk>
    ...
    ##%%%%%%%%%%%% calculating charge structure factor %%%%%%%%%%%%%%%%%%%%%
    ...
    ##Sq = Sq/max(Sq)
    #a, b, c=hexagonalBZmap(N1, N2, klist)
    return klist, Sq, nk
...
def plot_nk_in_hBz(N1, N2, ktot, nk, Label, Max=1):
    kBz, fig, ax = hexagonalBZmap(N1, N2, ktot)
    cm = plt.cm.get_cmap('turbo')
    mm = round(Max,2)
    if mm>Max:
        mm = round(Max-0.005,2)
    ticks=[0.0, round(mm/2,2), mm]
    ##ax.scatter(kBz[:,0], kBz[:,1],s=abs(connected_Sq)*1000, color='r', marker='h',edgecolor='black')
    sc=ax.scatter(kBz[:,0], kBz[:,1],c=nk, vmin=0, vmax=Max, s=170,cmap=cm, marker='h', alpha=0.8)
    cb = plt.colorbar(sc, fraction=0.02, pad=0.007, ticks=ticks)
    cb.ax.tick_params(labelsize=14)
    cb.ax.set_title(Label, fontsize=12)
    return fig, ax
...
N1, N2, Nup = 3, 3, 3
Vd = 10 ##float(sys.argv[3]) #displacement field
ktot, Sq, nk =  main(theta, N1, N2, Nup, Vd)

#figo, axo = plot_charge_density(nk, ktot, Nup)
fig_nk, ax_nk = plot_nk_in_hBz(N1, N2, ktot, nk, r'$n_k$')
fig, ax = plot_nk_in_hBz(N1, N2, ktot, Sq, r'$S(q)$', max(Sq))

#fig_nk.savefig('nk_in_mBZ_'+str(N1)+'x'+str(N2)+'_nh='+str(Nup)+'_ep='+str(epsilon_r)+'_theta='+str(round(theta,2))+'_Vd='+str(round(Vd,2))+'.pdf',bbox_inches='tight',dpi=300)
#fig.savefig('Sq_'+str(N1)+'x'+str(N2)+'_nh='+str(Nup)+'_ep='+str(epsilon_r)+'_theta='+str(round(theta,2))+'_Vd='+str(round(Vd,2))+'.pdf',bbox_inches='tight',dpi=300)
plt.show()
