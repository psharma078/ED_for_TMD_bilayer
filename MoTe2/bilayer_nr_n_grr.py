...
def main(theta, N1, N2, Nup, Vd):
    ...
    Nk = 5 #cutoff number [-Nk:Nk]*Q2 + [-Nk:Nk]*Q3
    #N1 = 4
    #N2 =4
    #Nup = 8
    ...
    def colorPlot(xx, yy, data,Label):
        cmap = 'jet'#'hot'
        figo,axo = plt.subplots(figsize=(3,2.4))
        ...
        c = axo.pcolormesh(xx/aM,yy/aM,data,shading = 'gouraud',cmap = cmap)
        cbar = plt.colorbar(c, fraction=0.05, pad=0.02, label=Label)
        cbar.set_label(label=Label[0], weight='normal', size=12)
        #plot top and bottom separate
        axo.tick_params(axis='both', labelsize=12)
        axo.set_aspect('equal')
        axo.set_xlabel(r'$x/a_M$',fontsize=12)
        axo.set_ylabel(r'$y/a_M$', fontsize=12)
        #ax.set_xlim(-1.6,1.6)
        #ax.set_ylim(-1.6,1.6)
        a1 = np.array([0,1])
        a2 = np.array([sqrt(3)/2,0.5])
        a12 = a1+a2
        alist = [[0,0],a1,a12,a2,[0,0]]
        for i in range(len(alist)-1):
            axo.plot([alist[i][0],alist[i+1][0]],[alist[i][1],alist[i+1][1]], color='k', linestyle='--')
        return figo,axo
    ...
    def real_space_density(klist, nk):
        #klist = bz_sampling(20,Q1,Q2)
        ...
        kr = 1j*(klist[:,0][:,np.newaxis]*x[np.newaxis,:] + klist[:,1][:,np.newaxis]*y[np.newaxis,:])

        #nr0 = np.sum(np.array(EvecList).conjugate(),axis=1)*np.sum(np.array(EvecList),axis=1)
        #nr0 = np.dot(nr0.flatten(),nk).real
        
        density_top = np.zeros((nx,ny))
        density_bot = np.zeros((nx,ny))
        for jj, Evec in enumerate(EvecList):
            Evec = Evec.flatten()
            wf_top = np.dot(Evec[:halfdim],exp(expfac+kr[jj]))
            wf_bot = np.dot(Evec[halfdim:],exp(expfac+kr[jj]))
            wf_top = np.reshape(wf_top,(nx,ny))
            wf_bot = np.reshape(wf_bot,(nx,ny))
            density_top += np.abs(wf_top)**2*nk[jj]
            density_bot += np.abs(wf_bot)**2*nk[jj]

        density_top = density_top /len(klist)
        density_bot = density_bot /len(klist)
        density = density_top + density_bot
        
        print('nbot_max, ntop_max, ntop/nbot =', density_bot.max(), density_top.max(), density_top.max()/density_bot.max(), density.max())

        #plot density
        cmap = 'jet'#'hot'
        figo,axo = plt.subplots(figsize=(3,2.4))
        figs, axs = plt.subplots(1,2,figsize=(6.3,2.4), sharey=True)
        axss = [axo,axs[0], axs[1]]
        Label = [r'$n_{tot}(r)$', r'$n_l(r)$']
        plt.subplots_adjust(left=0.1,
               bottom=0.1,
               right=0.9,
               top=0.9,
        ...
        c = axo.pcolormesh(xx/aM,yy/aM,density,shading = 'gouraud',cmap = cmap)
        cbar = plt.colorbar(c, fraction=0.05, pad=0.02, label=Label[0])
        cbar.set_label(label=Label[0], weight='normal', size=12)
        #plot top and bottom separate
        axs[0].pcolormesh(xx/aM,yy/aM,density_top,shading = 'gouraud',cmap = cmap, vmax=density_bot.max())
        c = axs[1].pcolormesh(xx/aM,yy/aM,density_bot,shading = 'gouraud',cmap = cmap, vmax=density_bot.max())
        plt.subplots_adjust(wspace=-0.25, hspace=0)
        cbar = figs.colorbar(c,ax=axs, shrink=1, fraction=0.04, pad=0.01, label=Label[1])
        cbar.set_label(label=Label[1], weight='normal', size=12)
        for i,ax in enumerate(axss):
            ax.tick_params(axis='both', labelsize=12)
            ax.set_aspect('equal')
            ax.set_xlabel(r'$x/a_M$',fontsize=12)
            if i!=2: ax.set_ylabel(r'$y/a_M$', fontsize=12)
            ax.set_xlim(-1.6,1.6)
            ax.set_ylim(-1.6,1.6)
        ...
        for i in range(len(alist)-1):
            axo.plot([alist[i][0],alist[i+1][0]],[alist[i][1],alist[i+1][1]], color='k', linestyle='--')
        return figo,axo, figs, axs, density
    ...
    def avr_nr(klist, nk):
        ...
        xy = np.vstack((x,y)).T
        a1 = np.array([0,1])*aM
        a2 = np.array([np.sqrt(3)/2,0.5])*aM
        xy = xy@np.array([a1,a2])
        xx,yy = xy[:,0].reshape(nx,ny), xy[:,1].reshape(nx,ny)
        x = xx.flatten()
        y = yy.flatten()
        solver = lambda k: EV_special(construct_ham(k))
        EvecList = Parallel(n_jobs=num_cores)(delayed(solver)(k) for k in klist)
        ...
        density = density_top + density_bot
        print('density sum=', np.sum(density,axis=(0,1))/N**2, density.max())
        fig, ax = colorPlot(xx,yy,density,r'$n(r)$') 
        plt.show()
        return density

    def two_point_grr(qlist, Sq):
        ##klist = bz_sampling(20,Q1,Q2)
        qlist = qlist @ np.array([Q1, Q2])
        nx = 100
        ny = 100
        ##calculate n=density nr        
        ro = np.array([0,0])
        xlist = np.linspace(-200,200,nx)*1e-10 - ro[0]#meter
        ylist = np.linspace(-200,200,ny)*1e-10 - ro[1]
        xx,yy = np.meshgrid(xlist,ylist)
        x = xx.flatten()
        y = yy.flatten()

        expfac = 1j*(qlist[:,0][:,np.newaxis]*x[np.newaxis,:] + qlist[:,1][:,np.newaxis]*y[np.newaxis,:])
        grr = np.einsum('qr,q->r',exp(expfac),Sq)
        grr = grr.real.reshape(nx,ny)/(Nup*Nup)
        print(grr.max())
        fig, ax = colorPlot(xx,yy,grr,r'$g(r-r_o)$')
        plt.show()
        return fig, ax

    def CSF(qval, evec, config, klist, Len, Fmat):#same order as Hamiltonian
        config_lookup = {value: index for index, value in enumerate(config)}
        k_ind = find_id_in_klist(klist)
        ...
        S_q = 0
        term1 = 0
        ## Eq. B3 of Lauchli paper but in the same order of operators as in the Hamiltonian
        for i, k1 in enumerate(k1_ind):
            config_k1 = getspin(config,k1)
            Lambda_k1 = Fmat[gk1_p_q_id[i]*n_k34**2 + k1*n_k34 + k1_plus_q_id[i]]
            if Len==1:
                for l in range(len(evec[0])):
                    term1 +=  abs(Lambda_k1)**2 * sum(evec[:,l].flatten().conjugate() * config_k1 * evec[:,l].flatten())
            else:
                term1 += abs(Lambda_k1)**2 * sum(evec.flatten().conjugate() * config_k1 * evec.flatten())

            valid_config_k1ID = (config_k1==1).nonzero()[0]
            f1, new_config = c_an(config[valid_config_k1ID], k1)

            for j, k2 in enumerate(k2_ind):
                if ((k1==k2) or (k2_minus_q_id[j]==k1_plus_q_id[i])): continue
                valid_config_k2ID = (getspin(new_config,k2)==1).nonzero()[0]
                f2, newconfig = c_an(new_config[valid_config_k2ID], k2)
                f2 = f1[valid_config_k2ID] * f2
                valid_config_k2mqID = (getspin(newconfig, k2_minus_q_id[j])==0).nonzero()[0]
                f3, newconfig = c_dag(newconfig[valid_config_k2mqID], k2_minus_q_id[j])
                f3 = f2[valid_config_k2mqID] * f3
                valid_config_k1pqID = (getspin(newconfig, k1_plus_q_id[i])==0).nonzero()[0]
                f4, newconfig = c_dag(newconfig[valid_config_k1pqID], k1_plus_q_id[i])
                fsign = f3[valid_config_k1pqID]* f4

                orginal_idlist = valid_config_k1ID[valid_config_k2ID[valid_config_k2mqID[valid_config_k1pqID]]]
                newconfig_id = np.array([config_lookup[nc] for nc in newconfig]).astype(int)

                Lambda_k2 = Fmat[gk2_m_q_id[j]*n_k34**2 + k2*n_k34 + k2_minus_q_id[j]]
                if Len == 1:
                    for l in range(len(evec[0])):
                        S_q += Lambda_k1*Lambda_k2 * sum(evec[:,l][newconfig_id].flatten().conjugate() * evec[:,l][orginal_idlist].flatten() * fsign)
                else:
                    S_q += Lambda_k1*Lambda_k2 * sum(evec[newconfig_id].flatten().conjugate() * evec[orginal_idlist].flatten() * fsign)

        #S_q += term1
        return S_q

    def CSF_for_degenrateGS(qval, evecs, configs, klist, Fmat):
        Len = len(configs)
        Sq = 0
        if Len==1:
            Sq = CSF(qval, evecs, configs[0], klist, Len, Fmat)
            Sq = Sq.real/len(evecs[0])
        else:
            for p in range(Len):
                Sq += CSF(qval, evecs[p], configs[p], klist, Len, Fmat)
            Sq = Sq.real/Len
        return Sq

    klist, qlist = sample_k_q(N1,N2,Nk)
    Vlist = coulomb_potential(qlist)
    Fmat_up, EEup = compute_F_mat(klist)
    ...
    #Gs_Ktot_ind = [4,9,17]#insert the k index (=k1*N2 + k2)  where gs degeneracy happens
    Gs_Ktot_ind = [0,5,7]#insert the k index (=k1*N2 + k2)  where gs degeneracy happens
    configGid = []
    configurs = []
    ...
    ##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #figl, axl, figs, axs, nr_nr0 = real_space_density(klist,nk)
    ##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #nr = avr_nr(klist, nk)
    ## DENSITY DENSITY CORRELATION
    ##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    qind = ((abs(qlist[:,0])<10) & (abs(qlist[:,1])<10)).nonzero()[0]
    qlist= qlist[qind]
    solver = lambda q: CSF_for_degenrateGS(q, Evecs, configurs, klist, Fmat_up)
    Sq =  np.array(Parallel(n_jobs=num_cores)(delayed(solver)(q) for q in klist))
    #for j in range(len(klist)):
    #    print(qlist[j], Sq[j])
    fig, ax = two_point_grr(klist, Sq)
    plt.show()
    ##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    sys.exit()
...
def density_nk(kidx, configs, evecs):
    #nk = np.trace(Op_rho @ Op_nk).real/np.trace(Op_rho).real
    nk = 0
    nstate = 0
    Len = len(configs)
...
fig, ax, figs, axs =  main(theta,Nx, Ny, Nhole, Vd)

#fig.savefig('real_space_density_'+str(Nx)+'x'+str(Ny)+'_nh='+str(Nhole)+'_ep='+str(epsilon_r)+'_theta='+str(round(theta,2))+'_Vd='+str(Vd)+'.pdf',bbox_inches='tight',dpi=300)
#figs.savefig('real_space_densities_2layer_'+str(Nx)+'x'+str(Ny)+'_nh='+str(Nhole)+'_ep='+str(epsilon_r)+'_theta='+str(round(theta,2))+'_Vd='+str(Vd)+'.pdf',bbox_inches='tight',dpi=300)
