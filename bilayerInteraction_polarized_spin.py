...
#             density = densit
    ...
    #for i in range(20):
    #    print(k1234_upup[i], Vc_upup[i])
    #print("len", len(k1234_upup), len(Vc_upup))
    ...
    # fig, ax = plot_manybody_energies(Ktot, N1, Evals)


    #Chern number calculation
    #dimH = 2* (2*Nk+1)**2 # Hamitonian dimension
    #bandidx = [-1, -2, -3] # compute the highest three bands' chern number
    #cn= chernnum(20,dimH,construct_ham, Q1, Q2, bandidx) 
    #print("Chern numbers from top:", cn)
    # band structure
    #fig, ax = bandstructure(energy_range = [-250,100])
    # # Electron/hole density at the top band (band_id = 0)
    #fig2, ax2 = electron_density(nbz = 20, band_id = 0)
...
Ktot_yang, Evals_yang=  main(theta,Nx, Ny, Nhole, Vd)
fig_yang,ax_yang = plot_ED_energies(Ktot_yang, Evals_yang)
#fig_yang.savefig('ED_6x3_nh='+str(Nhole)+'_ep='+str(epsilon_r)+'_theta='+str(round(theta,2))+'_Vd='+str(Vd)+'.png',bbox_inches='tight',dpi=300)
